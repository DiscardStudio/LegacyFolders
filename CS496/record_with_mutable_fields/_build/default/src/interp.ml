...
and
  ...
  | SetField(e1,id,e2) ->
    eval_expr e1 >>= fun obj ->
    eval_expr e2 >>= fun v ->
    let deconstruct = fun t ->
    (match t with
    | RecordVal(li) -> li
    | _ -> [])
    in
    let rec x = fun t id nv ->
      match t with 
      | RecordVal((m,(t,ex))::tl) ->
      if m = id
      then if t
            then RecordVal((m,(t,nv))::[])
            else failwith "Field not mutable"
      else RecordVal((m,(t,ex))::(deconstruct (x (RecordVal(tl)) id nv)))
      | _ -> RecordVal([])
    in let n = (x obj id v)
    in return @@ n
  | Unit -> return UnitVal
