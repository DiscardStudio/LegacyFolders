...
let rec lookup_class : string -> class_env -> class_decl = fun c_name c_env ->
  match c_env with
  | [] -> failwith "lookup_class: class c_name not found"
  | (id,c)::tl ->
    if c_name=id
    then c
...
let new_env : string list -> env ea_result = fun fs ->
  let rec f: string list -> int -> env = fun sl x -> (match sl with
  | [] -> EmptyEnv
  | h::tl -> (ExtendEnv(h,(RefVal(x)),(f tl (x+1)))))
  in return (f (List.rev fs) 0)
...
let lookup_method : string -> string -> class_env ->
  method_decl option = fun c_name m_name c_env ->
    (match lookup_class c_name c_env with
    | (_,_,(s2,m2)::tl)->(List.assoc_opt m_name ((s2,m2)::tl))
    | _ -> None)
...
and
  ...
  (* SOOL operations *)
  | NewObject(c_name,es) ->
    (match lookup_class c_name !g_class_env with
    | (_,f,methods) -> 
      let m = (List.assoc_opt "initialize" methods) in
      new_env f >>= fun env ->
      (match m with 
      | None ->  return (ObjectVal(c_name, env))
      | Some m -> sequence (List.map eval_expr es) >>= fun args ->
        apply_method "initialize" (ObjectVal(c_name, env)) args m >>= fun i ->
  ...
  | Super(m_name,es) ->
    ...
    (match lookup_method c_name m_name !g_class_env with
    | None -> error "Method not found"
    | Some m -> apply_method m_name self args m >>= fun i ->
      return @@ i)
