...
let rec foldt f a t =
  failwith "implement"
(* Quiz 2 *)
(* Creators: Michael Sanchez, Simrun Heir
...
let rec prune n t =
  ...
  match t with
  | Empty -> Empty
  | Node(d, Empty, Empty) -> Node(d, Empty, Empty)
  | Node(d,ln,rn) ->
    if n == 1
    then Node(d,Empty,Empty)
    else
    Node(d,(prune (n-1) ln),(prune (n-1) rn))
...
let rec perfect t =
  ...
  | Node(_, Empty,rl) -> false
  | Node(_, lr, Empty) -> false
  | Node(_, lr, rl) -> (perfect lr) && (perfect rl) && ((height lr)==(height rl))